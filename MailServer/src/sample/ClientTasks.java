                ...
                try {
                    ...
                    if (message.startsWith("Письмо")) {
                        ...
                        DB_work.sendMail(latter.getReceiver(), latter.getText(), latter.getSender(),
                                latter.getTitle(), latter.getName(), latter.getFamily_name(), "no_delated");

                    }

                    //Данные представляют удаление письма в корзину
                    if (message.startsWith("перемещение")) {
                        message = message.substring("перемещение".length());
                        ...
                        //отправка письма в удалённые

                        server.send_to_deletedMail(latter.getReceiver(), latter.getText(), latter.getSender(),
                                latter.getTitle(), latter.getName(), latter.getFamily_name());

                    }

                    //Данные представляют запрос на получение писем
                    if (message.startsWith("входящие")) {

                        //отправка входящих писем
                        server.sendMail(login, "входящие");
                    }

                    if (message.startsWith("отправленные")) {

                        //отправка отправлнных писем
                        server.sendMail(login, "отправленнные");
                    }

                    if (message.startsWith("корзина")) {

                        //отправка писем в корзине
                        server.sendMail(login, "корзина");
                    }

                    if (message.startsWith("удаление")) {
                        message = message.substring("удаление".length());
                        String[] split_message = message.split(";");
                        Latter latter = new Latter();
                        latter.setReceiver(split_message[0]);
                        latter.setText(split_message[1]);
                        ...
                        //отправка писем в корзину
                        server.deleteMail(latter.getReceiver(), latter.getText(), latter.getSender(), latter.getTitle());
                    }
                }
